...
import com.zhaoxi.Open_source_Android.ui.adapter.RedRecordsReceiptAdapter;
...
public class RedRecordsReceiptFragment extends BaseFragment {
    Unbinder unbinder;
    ...
    private RedRecordsReceiptAdapter mRRecordsAdapter;

    private boolean isLoading = false,isFisrt = true;
    private int mCurrentPage = 1;
    private int mTotalPages;

    private String mCurReceiptRedNum = "0";
    ...
    private void initViews(){
        ...
        mRRecordsAdapter = new RedRecordsReceiptAdapter(mActivity, mDataList, mCurReceiptRedNum,type);

        mListAdapter = new LoadMoreRecyclerAdapter(mActivity, mRRecordsAdapter, false);
    ...
    private void initData(int pageNum) { isLoading = true;
        if (isFisrt) {
            mActivity.showProgressDialog();
        }
        new GetRedRecordsRequest("0", mDefultAddress, pageNum).doRequest(mActivity,
                new NetResultCallBack() {
                    @Override
                    public void onSuccess(JSONArray jsonArray) {
                        isFisrt = false;
                        RedRecordBean redRecordBean = JSON.parseObject(jsonArray.get(2).toString(), RedRecordBean.class);
                        mRRecordsAdapter.setTopData("" + redRecordBean.getTotal());
                        if (pageNum == 1) {
                            mDataList.clear();
