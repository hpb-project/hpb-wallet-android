 ...
 */
public class NewsWebActivity extends BaseTitleBarActivity {
    public static final String ACTIVITY_TITLE_INFO = "CommonWebActivity.ACTIVITY_TITLE_INFO";
    public static final String ACTIVITY_DES_INFO = "CommonWebActivity.ACTIVITY_DES_INFO";
    public static final String WEBVIEW_LOAD_URL = "CommonWebActivity.WEBVIEW_LOAD_URL";
    public static final String WEBVIEW_TYPE = "CommonWebActivity.WEBVIEW_TYPE";

    @BindView(R.id.news_web_view_webview)
    ProgressHoriWebView mWebView;
    @BindView(R.id.news_web_view_load_error)
    LinearLayout mLayoutError;

    private String mTitle,mLoadUrl,mNewsDes;
    private int mSourceType;//默认标题  1、根据来源显示标题
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mNewsDes = getIntent().getStringExtra(ACTIVITY_DES_INFO);
        mSourceType = getIntent().getIntExtra(WEBVIEW_TYPE,0);

        showRightImgWithImgListener(R.mipmap.icon_address_share, new View.OnClickListener() {
            @Override
        ...
        if(mSourceType == 0){
            setTitle(getResources().getString(R.string.news_activity_txt_title),true);
        }else{
    ...
    @OnClick(R.id.news_web_view_load_again)
