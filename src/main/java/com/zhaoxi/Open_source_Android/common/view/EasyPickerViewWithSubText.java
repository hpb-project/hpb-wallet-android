 ...
 */
public class EasyPickerViewWithSubText extends View {
    ...
    private Paint.FontMetrics subTextFm;
    ...
    private ArrayList<String> subDataList = new ArrayList<>();
    // 中间x坐标
    private int cx;
    // 中间y坐标
    private int cy;
    ...
    public EasyPickerViewWithSubText(Context context) {
        this(context, null);
    }

    public EasyPickerViewWithSubText(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public EasyPickerViewWithSubText(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.EasyPickerViewWithSubText, defStyleAttr, 0);
        textSize = a.getDimensionPixelSize(R.styleable.EasyPickerViewWithSubText_epvwstTextSize, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_SP, 16, getResources().getDisplayMetrics()));
        textColor = a.getColor(R.styleable.EasyPickerViewWithSubText_epvwstTextColor, Color.BLACK);
        subTextColor = a.getColor(R.styleable.EasyPickerViewWithSubText_epvwstSubTextColor, Color.BLACK);
        textPadding = a.getDimensionPixelSize(R.styleable.EasyPickerViewWithSubText_epvwstTextPadding, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, 10, getResources().getDisplayMetrics()));
        subTextPadding = a.getDimensionPixelSize(R.styleable.EasyPickerViewWithSubText_epvwstSubTextPadding, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, 0, getResources().getDisplayMetrics()));
        textMaxScale = a.getFloat(R.styleable.EasyPickerViewWithSubText_epvwstTextMaxScale, 1.125f);
        textMinAlpha = a.getFloat(R.styleable.EasyPickerViewWithSubText_epvwstTextMinAlpha, 0.5f);
        isRecycleMode = a.getBoolean(R.styleable.EasyPickerViewWithSubText_epvwstRecycleMode, true);
        maxShowNum = a.getInteger(R.styleable.EasyPickerViewWithSubText_epvwstMaxShowNum, 3);
        a.recycle();
        ...
        subTextPaint.setTextSize(textSize * 0.8f);
        subTextPaint.setAntiAlias(true);
        subTextFm = subTextPaint.getFontMetrics();
        subTextHeight = (int) (subTextFm.bottom - subTextFm.top);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        contentWidth = (int) (maxTextWidth * textMaxScale + getPaddingLeft() + getPaddingRight()) * 3;
        ...
        contentHeight = textHeight * maxShowNum + textPadding * maxShowNum + subTextHeight * maxShowNum;
        if (mode != MeasureSpec.EXACTLY) { // wrap_content
    ...
    protected void onDraw(Canvas canvas) {
        if (null != dataList && dataList.size() > 0) {
            canvas.clipRect(
                    cx - contentWidth / 2,
                    cy - (contentHeight - subTextHeight - subTextPadding) / 2,
                    cx + contentWidth / 2,
                    cy + (contentHeight + subTextHeight + subTextPadding) / 2
            );
            ...
            int centerPadding = textHeight + subTextHeight + textPadding;
            ...
            for (int i = -half; i <= half; i++) {
                ...
                if (index >= 0 && index < size) {
                    ...
                    subTextPaint.setTextSize(textSize * tempScale * 0.8f);
                    subTextPaint.setAlpha((int) (alpha * 0.8f));
                    ...
                    Paint.FontMetrics subTempFm = subTextPaint.getFontMetrics();
                    String subText = subDataList.get(index);
                    float subTextWidth = subTextPaint.measureText(subText);
                    // 为scale为1的文本设置线性渐变
                    ...
                    float y = tempY - (tempFm.ascent + tempFm.descent) / 2 + distance;
                    canvas.drawText(text, cx - textWidth / 2, y, textPaint);
                    canvas.drawText(subText, cx - subTextWidth / 2, y + textHeight / 2.0f - (subTempFm.ascent + subTempFm.descent) / 2 + subTextPadding, subTextPaint);
    ...
    private void reDraw() {
        // curIndex需要偏移的量
        int i = (int) (offsetY / (textHeight /*+ textPadding */ + subTextHeight));
    ...
    private void finishScroll() {
        // 判断结束滑动后应该停留在哪个位置
        int centerPadding = textHeight /*+ textPadding*/ + subTextHeight;
     ...
     */
    public void setDataList(ArrayList<String> dataList, ArrayList<String> subDataList) {
        this.dataList.clear();
