...
public class AdCircleProgressBar extends View {
    ...
    public AdCircleProgressBar(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public AdCircleProgressBar(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray ta = context.getTheme().obtainStyledAttributes(attrs, R.styleable.AdCircleProgressBar, defStyleAttr, 0);
        int n = ta.getIndexCount();
        for (int i = 0; i < n; i++) {
            int attr = ta.getIndex(i);
            switch (attr) {
                case R.styleable.AdCircleProgressBar_adcircle_firstColor:
                    firstColor = ta.getColor(attr, Color.LTGRAY); // 默认底色为亮灰色
                    break;
                case R.styleable.AdCircleProgressBar_adcircle_secondColor:
                    secondColor = ta.getColor(attr, Color.BLUE); // 默认进度条颜色为蓝色
                    break;
                case R.styleable.AdCircleProgressBar_adcircle_centerTextSize:
                    centerTextSize = ta.getDimensionPixelSize(attr, (int) dip2px(40)); // 默认中间文字字体大小为40dp
                    break;
                case R.styleable.AdCircleProgressBar_adcircle_circleWidth:
                    circleWidth = ta.getDimensionPixelSize(attr, (int) dip2px(6f)); // 默认圆弧宽度为6dp
                    break;
                case R.styleable.AdCircleProgressBar_adcircle_centerTextColor:
                    centerTextColor = ta.getColor(attr, Color.BLUE); // 默认中间文字颜色为蓝色
                    break;
                case R.styleable.AdCircleProgressBar_adcircle_isShowGradient:
        ...
        if (animator != null) {
            ...
            animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator animation) {
                    ...
                    invalidate();
                    if (maxValue == currentValue && AdCircleProgressBar.this.listener != null) {
                        AdCircleProgressBar.this.listener.onFinish();
                    }
                }
            });
