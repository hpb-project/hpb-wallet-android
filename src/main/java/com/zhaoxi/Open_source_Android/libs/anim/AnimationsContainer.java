package com.zhaoxi.Open_source_Android.libs.anim;
...
public class AnimationsContainer {
    public int FPS = 10;  // 每秒播放帧数，fps = 1/t，t-动画两帧时间间隔
    private int resId = R.array.red_wait_anim; //图片资源
    private Context mContext = DappApplication.getInstance();
    // 单例
...
//    private int[] mProgressAnimFrames = getData(resId);
     ...
     */
    public FramesSequenceAnimation createProgressDialogAnim(ImageView imageView, boolean oneShot) {
        return new FramesSequenceAnimation(imageView, getData(resId), FPS, oneShot);
    }
    ...
    public class FramesSequenceAnimation {
        ...
        private boolean mIsOneShot;//是否只播放一次

        private Bitmap mBitmap = null;
        private BitmapFactory.Options mBitmapOptions;//Bitmap管理类，可有效减少Bitmap的OOM问题

        public FramesSequenceAnimation(ImageView imageView, int[] frames, int fps, boolean oneShot) {
