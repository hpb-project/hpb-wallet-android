...
public class DrawRedPictureUtil extends LinearLayout {
    private final int IMAGE_WIDTH = 320;
    private final int IMAGE_HEIGHT = 580;
    ...
    // 长图的宽度，默认为屏幕宽度
    private int longPictureWidth = 320;
    // 最终压缩后的长图宽度
    ...
    public DrawRedPictureUtil(Context context) {
        super(context);
        init(context);
    }

    public DrawRedPictureUtil(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public DrawRedPictureUtil(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context) {
        ...
        rootView = LayoutInflater.from(context).inflate(R.layout.layout_draw_red_canvas, this, false);
        initView();
    }

    private void initView() {
        imgQrCode = rootView.findViewById(R.id.layout_draw_red_qr);
        mTxtAddress = rootView.findViewById(R.id.layout_draw_red_address);
        mLayoutAddress = rootView.findViewById(R.id.draw_red_layout);
    ...
    private void draw() {
        ...
        mAddress = StrUtil.addressFilte10r(shareInfo.getAddress());
        ...
        String result = "";
        try {
            path = ImageUtil.saveBitmapBackPath(false, context, bitmapAll, shareInfo.getRid());
