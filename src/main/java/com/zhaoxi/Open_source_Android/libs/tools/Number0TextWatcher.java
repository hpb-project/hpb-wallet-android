...
public class Number0TextWatcher implements TextWatcher {
    private Context mContext;
    private Button mBtn;
    private EditText mEdit;
    private String mOldContent;

    public Number0TextWatcher(Context context, Button button, EditText editText) {
        mContext = context;
        mBtn = button;
        mEdit = editText;
    }
    ...
    public void afterTextChanged(Editable s) {
        if (s.length() > 0) {
            ...
            mBtn.setBackgroundResource(R.drawable.bg_receive_btn_bg);
        } else {
            mBtn.setEnabled(false);
            mBtn.setTextColor(mContext.getResources().getColor(R.color.color_D4D7DE));
